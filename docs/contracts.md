...
### Forward-to-address (as shipped, `claude/feature/forward-to-address`)
...
- Inngest function `artifacts-expire`: cron `0 4 * * *`, steps `purge-artifacts` (`ArtifactStore.listExpired(200)` → `purge`), `purge-inbound-rows` (rejected/failed older than 90 days).
...
- Needs from other packages beyond the contract above (integration): `inbound.listRecent` rows shaped like the spec SQL (`InboundMessageRow` in `apps/web/lib/server/phase1-stubs-inbound.ts`); a cross-tenant way to delete old rejected/failed inbound rows for the cron (stubbed as `purgeOldInboundMessages`); `ArtifactStore.listExpired`/`purge` must work without a tenant (cron).
### Dashboard and playbooks HTTP contract (as shipped, `_specs/dashboard.md`, `_specs/incident-playbooks.md`)
...
- `GET /api/verdicts/summary?sinceDays=7|30|90&userId` (default 30) → `{ sinceDays, total, byLabel, bySubjectType, topIndicators: [{ category, count }] (top 8), topDomains: [{ domain, count }] (top 8, excluding likely_safe), perDay: [{ day: "YYYY-MM-DD", malicious, suspicious, likely_safe, insufficient_evidence }] }` (perDay may be sparse; the UI zero-fills). Same 400/403/404 rules.
- `GET /api/verdicts/[id]` → list item fields + `body: Verdict`, `conversation: { id, title } | null`, `artifact: { id, kind, filename, mimeType, sizeBytes, expiresAt, expired } | null`, `inbound: { status, receivedAt, forwardedBy } | null`, `memberName`. 404 `not_found` for malformed ids, other tenants' verdicts and, for members, other members' verdicts.
...
- `GET /api/household` → `{ tenantId, name, role, members: [{ userId, name, email, role }] }`; `email` is `null` for every member unless the caller is an owner.
...
- Pages: `/dashboard`, `/verdicts/[id]`; `/` redirects signed-in users to `/dashboard`. Chat entry points: `/chat?playbook=<id>` (auto-sends "I think I …. Help me." with `playbook`), `/chat?verdict=<id>` (auto-sends "Tell me more about this check: …" with `verdictId`), `/chat?check=<url>` (pre-fills the composer only). Evidence links point at `GET /api/artifacts/<id>` (inline) and `?download=1`.
